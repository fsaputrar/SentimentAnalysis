...
class ensemble_features:
    ...
    def F11(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\kamus\emoticon_id.txt", "rb") as f:
    ...
    def F13(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\Postag\Noun.csv",
                  "rb") as n:
    ...
    def F14(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\Postag\Adjective.csv",
                  "rb") as adjv:
    ...
    def F15(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\Postag\Verb.csv",
                  "rb") as v:
    ...
    def F16(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\Postag\Adverb.csv",
                  "rb") as adv:
    ...
    def F17(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\Postag\Interjection.csv",
                  "rb") as i:
    ...
    def F23(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\kamus\sentiwords_id.txt", "rb") as f:
    ...
    def F24(self):
        with open("C:\Users\ACER\PycharmProjects\ProgramSkripsi\kamus\sentiwords_id.txt",
                  "rb") as f:
            wordsReader = csv.reader(f, delimiter=":")
            words = []
            for word in wordsReader:
    ...
    def F37(self):
        with open('C:\Users\ACER\PycharmProjects\ProgramSkripsi\kamus\penegasan_boosterwords_id.txt',
                  'rb') as f:
            wordsReader = csv.reader(f, delimiter=":")
            words = []
            for word in wordsReader:
    ...
    def Bag_of_Words(self, listkata):
        print "Fitur Bag of Words"
        print "# casefolding"
        for doc in listkata:
            casefolding = doc[1].lower()
            linkRegex = "((http|https|ftp)\:\/\/)?[a-zA-Z0-9\.\/\?\:@\-_=#]+\.([a-zA-Z]){2,6}([a-zA-Z0-9\.\&\/\?\:@\-_=#])*"
        ...
        for kata in listkata:
            kata[1] = stemmer.stem(str(kata[1].lower()))
            print kata[1]

        # filtering
        print "# filtering"
        stopword = open('C:\Users\ACER\PycharmProjects\ProgramSkripsi\kamus\modifikasi_stopword.txt', 'r').read()
        stopwordlist = re.split(r'\n', stopword)
        for kolom in listkata:
            wordstop = re.split(r'\s', kolom[1])
            fil = ''
            for y in wordstop:
                if y not in stopwordlist:
                    fil += ' ' + y
            kolom[1] = fil
            print kolom[1]
    ...
    #cek kata unik sebagai fitur
        print "# kata unik"
        unique = []
        for token in listkata:
            for unik in token[1]:
                if unik not in unique:
                    unique.append(unik)
            fiturkata = sorted(unique)
        print "unik = ", unique
        ...
        doc_count = []
        tf = []
        for kata in fiturkata: #mengambil setiap kata dalam set
            doc_count.append(0)
            raw_term =[]
            for f_doc in self.docs: #menghubung
                kt = f_doc[1].count(str(kata))
                raw_term.append(kt)
            tf.append(raw_term)
            a = len(fiturkata)
            print "fitur  (",kata,") = ",raw_term
            print "banyaknya= ", a
        return fiturkata, tf

    def allfitur(docs):
        x = ensemble_features(docs)
        ...
        kata, raw_term = x.Bag_of_Words(docs)
        for i in range(0, len(kata)):  # perulangan dari kata pertama yaitu idx 0 sampai kata terakhir
            features.append(raw_term[i])
            # print "raw_term[",i,"] = ", raw_term[i]
            i += 1

        # i = 1
        # for i in range(1,len(fiturs)):
        #     print "row fitur [",i,"] = ",fiturs[i]

        # simpan ke hasil ke file fiturs.csv
        ofile = open('fitursdata.csv', "wb")
        # ofile = open('fiturs.csv', "wb")
...
with open("datasetfix-revisi.csv","rb") as f:
